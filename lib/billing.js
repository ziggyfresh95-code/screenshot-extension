...
export async function getSubscription() {
  const rows = await rest(SUBS_TABLE, {
    query: q({
      select:
        'status,current_period_end,cancel_at_period_end,price_id',
    }),
  });
  return rows[0] || { status: 'inactive', cancel_at_period_end: false };
}
...
// Asks the create-portal-session Edge Function for a Stripe Customer Portal URL
// where the user can cancel their subscription. Returns the URL.
export async function openBillingPortal() {
  const { url, anonKey } = await getConfig();
  const session = await getSession();
  if (!session) throw new Error('Please sign in first.');

  const endpoint = `${url.replace(/\/+$/, '')}/functions/v1/create-portal-session`;
  let res;
  try {
    res = await fetch(endpoint, {
      method: 'POST',
    ...
    throw new Error('Could not reach the billing service. Check your network.');
  }
  ...
  if (!res.ok || !data.url) {
    throw new Error(
      data.error ||
        'Could not open billing. Is the create-portal-session function deployed?'
    );
  }
  return data.url;
}
